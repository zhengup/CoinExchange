...
// 一个简单的 ORM 工具类，封装了基于 JdbcTemplate 的数据库操作。
// 它通过扫描实体类并生成 Mapper 对象，实现了 SQL 的自动生成和执行。
// 适用于需要对数据库进行增删改查的场景，同时支持实体类的动态扫描和 DDL 导出。
@Component
public class DbTemplate {
    ...
    private static List<Class<?>> scanEntities(String basePackage) {
        ...
        //只过滤被@Entity标记的实体类，event match order
